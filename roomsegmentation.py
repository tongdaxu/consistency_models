...
colors = loadmat('/NEW_EDS/JJ_Group/zhuzr/semantic-segmentation-pytorch/data/color150.mat')['colors']
names = {}
with open('/NEW_EDS/JJ_Group/zhuzr/semantic-segmentation-pytorch/data/object150_info.csv') as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
...
def visualize_result(pred, path):

    # print predictions in descending order
    _, pred = torch.max(pred, dim=1)
    pred = pred.squeeze(0)
    pred = np.int32(pred.cpu())
    pixs = pred.size
    ...
    Image.fromarray(pred_color).save(path)
...
class SegmentationModule(SegmentationModuleBase):
    ...
    def forward(self, data, *, segSize=None):
